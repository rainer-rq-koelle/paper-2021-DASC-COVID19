...
#Normalized by the median of the last 3 months
temp2 <- temp1 %>% pivot_longer(cols = 2:5, names_to = "ROUTE", values_to = "FLIGHTS") %>% group_by(ROUTE) %>% mutate(MOVING_MEDIAN = quantile(FLIGHTS[month(DATE) %in% month(last(DATE))-1:month(last(DATE))], probs = 0.5), NORM_FLTS = FLIGHTS/MOVING_MEDIAN)
temp2
