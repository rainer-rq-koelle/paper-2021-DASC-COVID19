...
```{r}
#Joining coutries and regions
# NOTE: the airports.csv file below is the downloaded file from www.ourairports.org.

airports <- read_csv("data-raw/airports.csv") %>% transmute(ICAO = ident, CTRY = iso_country)
#There are missing airports
airports <- airports %>% add_row(ICAO  = c("SPJC", "YBMC", "LSZM", "YSCH", "EPLB", "K3M3", "VV01", "SC28", "CWF2", "EHDB", "74XA", "HE13"), CTRY  = c("PE", "AU", "FR", "AU", "PL", "US", "VN", "US", "CA", "NL", "US", "EG"))

#Joining to ADEP
fb2 <- left_join(fb1, airports, by = c("ADEP" = "ICAO")) %>% mutate(ADEP_CTRY = CTRY, .keep = "unused", .after = ADEP)
#Joining to ADES
fb3 <- left_join(fb2, airports, by = c("ADES" = "ICAO")) %>% mutate(ADES_CTRY = CTRY, .keep = "unused", .after = ADES)

#Check  NA's
colSums(is.na(fb3))
# Very few NA's - it's safe to drop and factor now

world_dataset <- fb3 %>% drop_na() %>% mutate(ADEP_CTRY = as.factor(ADEP_CTRY), ADES_CTRY = as.factor(ADES_CTRY))
world_dataset
```
